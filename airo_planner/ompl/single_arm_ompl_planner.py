...
from airo_planner import SingleArmPlanner, function_numpy_to_ompl, numpy_to_ompl_state, ompl_path_to_numpy
...
class SingleArmOmplPlanner(SingleArmPlanner):
    ...
    def __init__(
        ...
        joint_bounds: Tuple[JointConfigurationType, JointConfigurationType] | None = None,
        ...
        if joint_bounds is None:
            n_dofs = self.degrees_of_freedom
            self.joint_bounds = np.full(n_dofs, -2 * np.pi), np.full(n_dofs, 2 * np.pi)
        else:
    ...
    def _create_simple_setup(self) -> og.SimpleSetup:
        ...
        for ik_solution in ik_solutions_valid:
            ...
            if path is not None:
                ...
                if self._path_length is None:
                    raise ValueError("For devs: path length should not be None here.)")
